...
def convert_currency(amount, from_currency, to_currency):
    ...
    try:
        ...
        print("Error: Could not connect to the currency conversion service.")
        return None

def currency_conversion_menu():
    print("\nCurrency Conversion")
    amount_input = input("Enter the amount to convert: ")
    try:
        amount = float(amount_input)
    except ValueError:
...
def get_financial_summary(username):
    # Retrieve transactions from the database
    cursor.execute("SELECT type, amount FROM transactions WHERE username=?", (username,))
    transactions = cursor.fetchall()

    # Prepare data for the microservice
    data = [{'category': trans[0], 'amount': trans[1]} for trans in transactions]

    url = "http://127.0.0.1:5000/totals"
    try:
        ...
        print("Error: Could not connect to the financial summary service.")

def get_ranked_categories(username):
    # Retrieve transactions from the database
    cursor.execute("SELECT category, amount FROM transactions WHERE username=?", (username,))
    transactions = cursor.fetchall()
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("\nRanked Categories:")
            for category, total_amount in ranked_data:
                print(f"Category: {category}, Total Amount: ${total_amount:.2f}")
        else:
            print(f"Error: {response.text}")
    except requests.ConnectionError:
        print("Error: Could not connect to the ranking service.")


# Main Program Loop
def main_menu(username):
    while True:
        action = input("\nChoose an action:\n1. Add Income/Expense\n2. View Transactions\n3. View Financial Summary\n4. View Ranked Categories\n5. Edit Transaction\n6. Delete Transaction\n7. Currency Conversion\n8. Logout\n> ")
        if action == '1':
            add_transaction(username)
        elif action == '2':
            view_transactions_menu(username)
        elif action == '3':
            get_financial_summary(username)
        elif action == '4':
            get_ranked_categories(username)
        elif action == '5':
